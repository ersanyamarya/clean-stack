...
import { runCommandOnServer } from './actions/server';
...
configCommand.command('list').option('-l, --list', 'List view').option('-t, --table', 'Table view').action(listConfigAction);
configCommand.command('add').description('Add a new server from the ssh config').action(addAction);
configCommand.command('remove').description('Remove a server from configuration').action(removeAction);
configCommand.command('default').description('Set or view default server').action(defaultAction);
