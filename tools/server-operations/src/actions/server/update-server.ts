...
export const updateServer = async (options: UpdateServerOptions) => {
  ...
  logger.info(await executeBash(generateSSHCommand(serverToRunOn, `sudo apt update -y`)));

  logger.info(`Upgrading packages on ${serverToRunOn.name}`);
  logger.info(await executeBash(generateSSHCommand(serverToRunOn, `sudo apt upgrade -y`)));
  if (options.isRaspberryPi) {
    logger.info(`Upgrading firmware on ${serverToRunOn.name}`);
    logger.info(await executeBash(generateSSHCommand(serverToRunOn, `sudo rpi-update`)));
  }
  logger.info(`Auto removing packages on ${serverToRunOn.name}`);
  logger.info(await executeBash(generateSSHCommand(serverToRunOn, `sudo apt autoremove -y`)));

  logger.info(`Cleaning up packages on ${serverToRunOn.name}`);
  logger.info(await executeBash(generateSSHCommand(serverToRunOn, `sudo apt clean`)));
